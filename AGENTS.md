...
- Always run the full test suite (pytest) after any code change and fix failing tests rather than dismissing them as pre-existing 
— Do **not** consider a task complete until all tests pass.

## Debugging section

- When fixing a bug, identify the root cause before editing
— Verify the actual error source (e.g., run the failing command) rather than applying surface-level fixes.
